...
| Cobertura de controllers: todos os 6 controllers anteriormente em 0% agora possuem suítes unitárias dedicadas (`ai.controller.test.ts`, `knowledge.controller.test.ts`, `voiceOutbound.controller.test.ts`, `observability.controller.test.ts`, `organization.controller.test.ts`, `workflowCollab.controller.test.ts`) | ✅ Resolvido | Resolvido em 2026-09-21 — zero controllers sem cobertura automatizada |
...
- **2026-09-21** — Integração da Onda 6 (`origin/integracao/onda-6`) no `main` (26 commits). Resolução do handoff `05-para-00-transfer-dial-outcome-followup.md` com `dialStatusHandler` e callback no Twilio. Resolução total do achado de controllers em 0% de cobertura com a criação de suítes de teste para `ai.controller.ts`, `voiceOutbound.controller.ts`, `observability.controller.ts`, `organization.controller.ts` e `workflowCollab.controller.ts` (junto ao `knowledge.controller.test.ts` da Onda 6). Conexão de `pages/Dashboard/KnowledgeManager.tsx` às rotas reais da API de agentes e RAG com verificação antivírus ClamAV obrigatória e simulador de consulta RAG com testes de frontend (`KnowledgeManager.test.tsx`). Gate completo verificado e aprovado: `typecheck` (0 erros), `lint` (0 erros), `test` (683 testes aprovados em 95 suites), `build` (sucesso).

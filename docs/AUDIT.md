...
| Cobertura de controllers: reduzidos de 6 para 4 com 0% (`ai.controller.ts` 100% coberto em `src/controllers/ai.controller.test.ts` e `knowledge.controller.ts` coberto em `src/controllers/knowledge.controller.test.ts`; restam `observability.controller.ts`, `organization.controller.ts`, `voiceOutbound.controller.ts`, `workflowCollab.controller.ts`) | 🟡 Médio | Parcialmente resolvido em 2026-09-21 |
...
- **2026-09-21** — Integração da Onda 6 (`origin/integracao/onda-6`) no `main` (26 commits). Resolução do handoff `05-para-00-transfer-dial-outcome-followup.md` com `dialStatusHandler` e callback no Twilio. Cobertura de testes unitários para `ai.controller.ts` (`src/controllers/ai.controller.test.ts` com 15 testes cobrindo todos os endpoints: chat, consentimento LGPD, TTS, geração de música/vídeo e refactor/generate de workflow). Conexão de `pages/Dashboard/KnowledgeManager.tsx` às rotas reais da API de agentes e RAG com verificação antivírus ClamAV obrigatória e simulador de consulta RAG com testes de frontend (`KnowledgeManager.test.tsx`). Gate completo verificado e aprovado: `typecheck` (0 erros), `lint` (0 erros), `test` (91 suites, 661 testes aprovados), `build` (sucesso).
